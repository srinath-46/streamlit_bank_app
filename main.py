...
def admin_dashboard():
    ...
    if option == "📃 All Applications":
        ...
        st.subheader("Approve / Decline Loan Applications")
        ...
        for idx, row in pending_loans.iterrows():
            ...
            if risk_score <= 39:
                loans_df.loc[idx, "status"] = "approved"
                loans_df.loc[idx, "remarks"] = f"Auto-approved. {remark}"
                st.success(f"✅ {loan_id} auto-approved (Low Risk)")
            elif risk_score >= 61:
                loans_df.loc[idx, "status"] = "declined"
                loans_df.loc[idx, "remarks"] = f"Auto-declined. {remark}"
                st.error(f"❌ {loan_id} auto-declined (High Risk)")
            else:
        ...
        if st.button("Fetch Info"):
            ...
            if user_info.empty:
                ...
                loans = loans_df[loans_df['user_id'] == user_id]
                st.write("👤 Info", user_info.drop(columns=['password'], errors='ignore'))
                st.write("🏦 Account", acc)
                st.write("📄 Loans", loans)
                st.write("💸 Transactions", tx)
